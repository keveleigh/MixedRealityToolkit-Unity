...
{
	public abstract class DictionaryField<KeyType> : Frame where KeyType : class
	{
		#region Events

		public class ItemAddedEvent : EventBase<ItemAddedEvent>
		{
			public static ItemAddedEvent GetPooled(KeyType key)
			{
				var e = GetPooled();
				e.Key = key;
				return e;
		...
		{
			public static ItemRemovedEvent GetPooled(KeyType key)
			{
				var e = GetPooled();
				e.Key = key;
				return e;
		...
		private const string _invalidBindingError = "Invalid binding '{0}' for DictionaryField: property '{1}' is type '{2}' but should be an array";
		private const string _invalidTypeError = "Invalid item type '{0}' for DictionaryField: the item type must be a default constructable class when used with allowDerived = false";
		private const string _failedAddError = "Failed to add item '{0}' of type '{1}' to the DictionaryField: the item type must be a value type or default constructable class that is compatible with the dictionary";
		...
		public new const string UssClassName = "uxtools-dictionary-field";
		...
		private IDictionaryProxy<KeyType> _proxy;
		...
		private BaseField<KeyType> _addField;
		...
		public IDictionaryProxy<KeyType> Proxy => _proxy;
		public Type ItemType => _itemType;
		public bool AllowDerived => _allowDerived;

		public void SetProxy(IDictionaryProxy<KeyType> proxy, Type itemType, bool allowDerived)
		{
		...
		protected abstract BaseField<KeyType> CreateAddField();
		protected abstract void UpdateAddPlaceholder();
		...
		private void CreateElement(int index, KeyType key)
		{
		...
		private void CheckElement(int index, KeyType key)
		{
		...
		private void UpdateContent(VisualElement item, int index, KeyType key)
		{
			SetKey(item, key);
		...
		private KeyType GetKey(VisualElement element)
		{
			return element.userData is KeyType s ? s : null;
		}

		private void SetKey(VisualElement element, KeyType key)
		{
			element.userData = key;
		}

		protected abstract bool IsEmpty(KeyType key);

		private void AddKeyChanged(KeyType newValue)
		{
			var empty = IsEmpty(newValue);
		...
		public new class UxmlTraits : Frame.UxmlTraits
		{
			...
			public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)
			{
				...
				var dictionary = ve as DictionaryField<KeyType>;
	...
	public class StringDictionaryField : DictionaryField<string>
	{
		#region Members
		...
		public StringDictionaryField() : base()
		{
		}

		protected override BaseField<string> CreateAddField()
		{
			var addField = new TextField();
			addField.Q(TextField.textInputUssName).RegisterCallback<KeyDownEvent>(evt => AddKeyPressed(evt));

			_addPlaceholderText = new Placeholder(AddPlaceholder);
			_addPlaceholderText.AddToField(addField);

			return addField;
		}

		protected override bool IsEmpty(string key)
		{
			return string.IsNullOrEmpty(key);
		}

		protected override void UpdateAddPlaceholder()
		{
			_addPlaceholderText.text = AddPlaceholder;
		}
		...
		public new class UxmlFactory : UxmlFactory<StringDictionaryField, UxmlTraits> { }
		public new class UxmlTraits : Frame.UxmlTraits { }

		#endregion
	}

	public class EnumDictionaryField : DictionaryField<Enum>
	{
		#region Members

		private EnumField _addField;

		#endregion

		#region Public Interface

		public EnumDictionaryField() : base()
		{
		}

		public void Initialize(Enum defaultValue)
		{
			_addField.Init(defaultValue);
		}

		protected override BaseField<Enum> CreateAddField()
		{
			_addField = new EnumField(AddPlaceholder);
			_addField.labelElement.style.alignContent = Align.FlexEnd;
			return _addField;
		}

		protected override bool IsEmpty(Enum key) => key == null;

		protected override void UpdateAddPlaceholder()
		{
			_addField.label = AddPlaceholder;
		}
		...
		public new class UxmlFactory : UxmlFactory<EnumDictionaryField, UxmlTraits> { }
		public new class UxmlTraits : Frame.UxmlTraits { }
