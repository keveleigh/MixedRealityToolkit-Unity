...
namespace Microsoft.MixedReality.Toolkit.EditorTools
{
	public class PopupField<T> : BaseField<T>
	{
		...
		public const string UssClassName = "uxtools-popup-field";
		...
		private const string _invalidValuesError = "Invalid values for PopupField: Values must not be null and must have a Count > 0";
		private const string _invalidOptionsWarning = "Invalid Options for PopupField: the number of Options does not match the number of Values";
		private const string _missingValueWarning = "The value of PopupField did not exist in the list of values: Changing to the first valid value";
		...
		public override void SetValueWithoutNotify(T newValue)
		{
			...
			else if (ValidateValue(newValue))
			{
				base.SetValueWithoutNotify(newValue);
