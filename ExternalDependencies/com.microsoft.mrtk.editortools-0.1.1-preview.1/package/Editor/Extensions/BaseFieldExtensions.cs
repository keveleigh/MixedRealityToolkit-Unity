...
namespace Microsoft.MixedReality.Toolkit.EditorTools
{
    public static class BaseFieldExtensions
    {
        ...
        static BaseFieldExtensions()
        {
            ...
            if (configureFieldMethod != null)
            {
                if (configureFieldMethod.HasSignature(typeof(VisualElement), null, typeof(SerializedProperty)))
                {
                    _configureFieldMethod = configureFieldMethod;
                    _configureFieldInstance = new PropertyField();
                    _configureFieldParameters = new object[2];
                }
                else if(configureFieldMethod.HasSignature(null, null, typeof(SerializedProperty), null))
                {
                    _configureFieldMethod = configureFieldMethod;
                    _configureFieldInstance = new PropertyField();
                    _configureFieldParameters = new object[3];
                }
            }
        ...
        public static VisualElement ConfigureProperty<T>(this BaseField<T> field, SerializedProperty property)
        {
            ...
            if (_configureFieldParameters.Length > 2)
            {
